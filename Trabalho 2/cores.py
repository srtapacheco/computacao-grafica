...
def is_adjacent_cube_removed(i, j, k):
    # Verifica se existe um cubo adjacente removido
    adjacents = [(i-1, j, k), (i+1, j, k), (i, j-1, k), (i, j+1, k), (i, j, k-1), (i, j, k+1)]
    for adj_i, adj_j, adj_k in adjacents:
        if 0 <= adj_i < n and 0 <= adj_j < n and 0 <= adj_k < n:
            adj_cube = (adj_i * n + adj_j) * n + adj_k
            if adj_cube in removed:
                return False
    return False
...
def mousePressed(button, state, x, y):
    ...
    if state == GLUT_DOWN:
        ...
        if selected >= 0:
            ...
            j = (selected // n) % n
            k = selected % n
            if not is_adjacent_cube_removed(i, j, k):
...
def main():
    ...
    glutIdleFunc(idle)  # Adiciona a função idle para atualização contínua
    glutMainLoop()
