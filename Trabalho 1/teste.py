...
class Rect(object):
    ...
    def set_matrix(self, t):
        ...
        if determinant == 0:
            # Tratar o caso de matriz singular aqui
            return
...
class ShapeEditor(object):
    def __init__(self):
        ...
        self.center_x = 0
        self.center_y = 0
        self.original_center_x = 0
        self.original_center_y = 0
        ...
        if self.mode == "CREATE_RECTANGLE":
            ...
            for s in self.shapes:
                if s.contains([x, y]):
                    ...
                    self.last_x = x
                    self.last_y = y
                    self.center_x = (s.points[0][0] + s.points[1][0]) / 2
                    self.center_y = (s.points[0][1] + s.points[1][1]) / 2
                    self.original_center_x = self.center_x
                    self.original_center_y = self.center_y
                    break
    ...
    def mouse_drag(self, x, y):
        if self.mode == "CREATE_RECTANGLE":
            ...
            if self.selected_shape:
                ...
                t = create_from_translation([-self.center_x, -self.center_y, 0])
                r = create_from_axis_rotation([0, 0, 1], rotation_angle)
                t_inv = create_from_translation([self.center_x, self.center_y, 0])
                self.selected_shape.set_matrix(multiply(multiply(multiply(t_inv, r), t), self.selected_shape.m))
                self.last_x = x
                self.last_y = y
        elif self.mode == "SCALE":
            if self.selected_shape:
                dx = x - self.selected_shape.points[0][0]
                dy = y - self.selected_shape.points[0][1]
                scale_x = abs(dx) / abs(self.last_x - self.selected_shape.points[0][0])
                scale_y = abs(dy) / abs(self.last_y - self.selected_shape.points[0][1])
                t = create_from_scale([scale_x, scale_y, 1], self.selected_shape.points[0])
                determinant = np.linalg.det(t)
                if determinant == 0:
                    return
                self.selected_shape.set_matrix(multiply(self.selected_shape.m, t))
...
def main():
    ...
    glutCreateWindow(b"Shape Editor")
    shape_editor = ShapeEditor()
    glutReshapeFunc(shape_editor.reshape)
    glutMouseFunc(shape_editor.mouse)
    glutMotionFunc(shape_editor.mouse_drag)
    glutKeyboardFunc(shape_editor.keyboard)
    shape_editor.create_menu()
    glutDisplayFunc(shape_editor.display)
    glutMainLoop()
